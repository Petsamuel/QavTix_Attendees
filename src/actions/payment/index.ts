...
import { ADD_PAYMENT_CARD, ADD_PAYMENT_CARD_CONFIRM, PAYMENT_ACCOUNTS_ENDPOINT, PAYMENT_METHODS_ENDPOINT, SET_DEFAULT_PAYMENT_CARD_ENDPOINT } from "@/endpoints"
...
export async function getPaymentAccounts(): Promise<GetPaymentAccountsResult> {
    ...
    return _getPaymentAccounts(accessToken);
}

async function _getPaymentAccounts(accessToken: string | undefined): Promise<GetPaymentAccountsResult> {
    "use cache"
    cacheTag(CACHE_TAGS.PAYMENT_ACCOUNTS)
    try {
...
export async function getPaymentMethods(): Promise<PaymentMethodsResult> {
    const accessToken = (await cookies()).get("access_token")?.value;
    return _getPaymentMethods(accessToken);
}

async function _getPaymentMethods(accessToken: string | undefined): Promise<PaymentMethodsResult> {
    "use cache"
