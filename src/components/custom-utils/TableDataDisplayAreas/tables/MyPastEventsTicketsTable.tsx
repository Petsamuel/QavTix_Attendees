...
export default function MyPastEventsTicketsTable({
    ...
    if (!tickets.length) {
        ...
        return (
            <div className="my-10">
                <NoPastEvents
                    title="No Past Event Tickets"
                    href={`${process.env.NEXT_PUBLIC_APP_DOMAIN}/events`}
                    text="You don't have any past tickets yet. Start exploring amazing events!"
                />
            </div>
        )
    }
                        ...
                        <tbody className="divide-y divide-neutral-5 bg-white">
                            {tickets.map((item) => (
                                <tr key={item.id} className="hover:bg-brand-neutral-3/50 transition-colors group">
                                    ...
                                    <td className="p-4">
                                        <Badge className="border-[0.81px] border-brand-secondary-2 bg-[#F0F1F2] rounded-sm text-xs text-brand-secondary-4">
                                            Past
                                        </Badge>
                                    </td>
